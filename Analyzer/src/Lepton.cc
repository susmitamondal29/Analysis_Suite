...
bool Lepton::passZVeto() {
    for (auto tidx: tightList) {
        LorentzVector tlep(pt->At(tidx), eta->At(tidx), phi->At(tidx), mass->At(tidx));
        for (auto lidx: looseList) {
            if (tidx == lidx || charge->At(tidx)*charge->At(lidx) > 0)
                continue;
            LorentzVector llep(pt->At(lidx), eta->At(lidx), phi->At(lidx), mass->At(lidx));
...
bool Lepton::passJetIsolation(size_t idx, Particle& jets) {
    ...
    LorentzVector jetV(jets.pt->At(jidx), jets.eta->At(jidx), jets.phi->At(jidx), jets.mass->At(jidx));
